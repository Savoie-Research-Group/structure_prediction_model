...
from ensem_beam_search import beam_search

def bs_accuracy(batch, model_lst, tgt_maxlen, idx_to_ch_dict, target_start_token_idx, target_end_token_idx):
    count_top1 = 0
    count_top10 = 0
    beam_size = 10
    source, target = batch
    bs = source.size(dim=0)
    preds = beam_search(model_lst, source, beam_size, tgt_maxlen, target_start_token_idx, target_end_token_idx)
    batch_pred_10 = []
    for i in range(bs):
