...
def beam_search(model_lst, source, beam_width, tgt_maxlen, target_start_token_idx, target_end_token_idx):
    mode_lst = model_lst[0].mode_lst
    device = model_lst[0].device
    bs = source.size(dim=0)
    num_hid = model_lst[0].dim_model
    num_classes = model_lst[0].num_tokens
    if mode_lst[0] == 1:
        ms_enc_lst = []
        for model in model_lst:
            ms_enc = model.ms_input(source[:,0:999].long())
            ms_enc = model.ms_encoder(ms_enc)
            ms_enc_lst.append(ms_enc)
    if mode_lst[1] == 1:
        ir_enc_lst = []
        for model in model_lst:
            ir_enc = model.IR_input(source[:,999:1899].long())
            ir_enc = model.IR_encoder(ir_enc)
            ir_enc_lst.append(ir_enc)
    if mode_lst[2] == 1:
        nmr_enc_lst = []
        for model in model_lst:
            nmr_enc = model.nmr_input(source[:,1899:].long())
            nmr_enc = model.nmr_encoder(nmr_enc)
            nmr_enc_lst.append(nmr_enc)
    ...
    for _ in range(bs):
        ...
        for model in model_lst:
            if mode_lst[0] == 1:
                ...
                enc_input_ms_lst.append(enc_input_ms)
            ...
            if mode_lst[1] == 1:
                ...
                enc_input_ir_lst.append(enc_input_ir)
                ...
                if mode_lst[0] == 1:
                    ...
                    dec_out_final = dec_out_ir
            if mode_lst[2] == 1:
                enc_input_nmr = torch.reshape(nmr_enc[_,:,:],(1,993,emb_dim))
                enc_input_nmr_lst.append(enc_input_nmr)
                ...
                if mode_lst[0] == 0 & mode_lst[1] == 0:
                    dec_out_final = dec_out_nmr
                else:
                    dec_out_final = torch.cat((dec_out_final,dec_out_nmr),axis=2)
            prob_tensor = model.ded_classifier(dec_out_final)
            prob_tensor_lst.append(prob_tensor)

        prob_tensor = sum(prob_tensor_lst) / len(prob_tensor_lst)
        ...
        tgt_padding_mask = model_lst[0].create_pad_mask(next_ys,0)
        sequence_length = next_ys.size(1)
        tgt_mask = model_lst[0].get_tgt_mask(sequence_length).to(device)
        if mode_lst[0] == 1:
            for i in range(len(enc_input_ms_lst)):
                enc_input_ms_lst[i] = enc_input_ms_lst[i].repeat((beam_width, 1, 1))
        if mode_lst[1] == 1:
            for i in range(len(enc_input_ir_lst)):
                enc_input_ir_lst[i] = enc_input_ir_lst[i].repeat((beam_width, 1, 1))
        if mode_lst[2] == 1:
            for i in range(len(enc_input_nmr_lst)):
                enc_input_nmr_lst[i] = enc_input_nmr_lst[i].repeat((beam_width, 1, 1))

        for i in range(tgt_maxlen-2):
            prob_tensor_lst = []
            for index,model in enumerate(model_lst):
                if mode_lst[0] == 1:
                    dec_out_ms = model.ms_decoder(tgt = model.tgt_input(next_ys.long()),memory = enc_input_ms_lst[index],tgt_mask = tgt_mask,tgt_key_padding_mask = tgt_padding_mask)
                    dec_out_ms = model.ms_classifier(dec_out_ms)
                    dec_out_final = dec_out_ms
                if mode_lst[1] == 1:
                    dec_out_ir = model.IR_decoder(tgt = model.tgt_input(next_ys.long()),memory = enc_input_ir_lst[index],tgt_mask = tgt_mask,tgt_key_padding_mask = tgt_padding_mask)
                    dec_out_ir = model.ir_classifier(dec_out_ir)
                    if mode_lst[0] == 1:
                        dec_out_final = torch.cat((dec_out_final,dec_out_ir),axis=2)
                    else:
                        dec_out_final = dec_out_ir
                if mode_lst[2] == 1:
                    dec_out_nmr = model.nmr_decoder(tgt = model.tgt_input(next_ys.long()),memory = enc_input_nmr_lst[index],tgt_mask = tgt_mask,tgt_key_padding_mask = tgt_padding_mask)
                    dec_out_nmr = model.nmr_classifier(dec_out_nmr)
                    if mode_lst[0] == 0 & mode_lst[1] == 0:
                        dec_out_final = dec_out_nmr
                    else:
