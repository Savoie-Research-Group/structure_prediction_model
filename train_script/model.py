# This is the structure_transformer model, mode_lst (list with length 3) refers to whether MS, IR or H-NMR is existed in training

import torch
import torch.nn as nn
from embedding import PositionEmbedding

class structure_transformer(nn.Module):
    def __init__(
        self,
        vocab_size,
        num_tokens,
        ...
        if mode_lst[0] == 1:
            ...
            self.ms_classifier = nn.Linear(dim_model, num_tokens, bias=False)
        if mode_lst[1] == 1:
            ...
            self.ir_classifier = nn.Linear(dim_model, num_tokens, bias=False)
        if mode_lst[2] == 1:
            ...
            self.nmr_classifier = nn.Linear(dim_model, num_tokens, bias=False)
        self.ded_classifier = nn.Linear((mode_lst[0]+mode_lst[1]+mode_lst[2])*num_tokens, num_tokens, bias=False)
    ...
    def forward(
        ...
        if self.mode_lst[2] == 1:
            ...
            if (self.mode_lst[0] == 0) & (self.mode_lst[1] == 0):
                y_final = y_nmr
            else:
