...
def train_loop(model, opt, mode_lst, dataloader, num_tokens, device):
    model.train()
    gc.collect()
    total_loss = 0
    for batch in dataloader:
        ...
        pred = model(x_ms, x_ir, x_nmr, y_input)
...
def validation_loop(model, mode_lst, dataloader, num_tokens, device):
    model.eval()
    total_loss = 0
    with torch.no_grad():
        for batch in dataloader:
